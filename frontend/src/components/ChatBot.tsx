...
const RULES: { keywords: string[]; response: string }[] = [
  ...
  },
  {
    ...
    response:
      "🟡 **Open** — Your query has been received and is waiting for an admin to pick it up.",
  },
  {
    keywords: ["in progress", "inprogress", "being worked"],
    response:
      "🔵 **In Progress** — An admin is actively working on your query. A response should come soon.",
  },
  {
    keywords: ["resolved"],
    response:
      "✅ **Resolved** — Your issue has been addressed. If not fully resolved, submit a new query with more details.",
  },
  {
    keywords: ["closed"],
    response:
      "🔒 **Closed** — The query is archived. For follow-up issues, please submit a new query.",
  },
  {
  ...
  },
  {
    ...
    response:
      "🗑️ You can delete your queries from **My Queries** or the **Query Detail** page. Deleted queries cannot be recovered.",
  },
  {
  ...
  },
  {
    ...
    response:
      "📋 Go to the **Register** page from the login screen, fill in your name, username, and password to create an account.",
  },
  {
    keywords: ["admin", "contact admin", "speak to someone"],
    response:
      "👩‍💼 Submit a query to reach an admin. All communication goes through the query system — there's no direct messaging.",
  },
  {
...
export function ChatBot() {
  ...
  const sendMessage = (text: string) => {
    ...
    setTimeout(() => {
      ...
      const botMsg: Message = { id: Date.now() + 1, from: "bot", text: botReply };
  ...
  // Format newline characters in bot messages
